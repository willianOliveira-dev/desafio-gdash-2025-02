...
export class UsersController {
    ...
    @Get(':id')
    async findOneUser(@Param('id', ParseObjectIdPipe) id: string): Promise<ResponseApi<UserModelWithoutPassword>> {
        const user = await this.usersService.findOne(id);
        return { message: 'Usuário encontrado com sucesso.', data: user };
    }

    @Post()
    async createUser(@Body() dto: CreateUserDto): Promise<ResponseApi<UserModelWithoutPassword>> {
